...
class ECAPA_TDNN(nn.Module):
    def __init__(self, block, C, model_scale, log_input=True, num_mels=80, num_out=192, **kwargs):
        self.log_input  = log_input
        super(ECAPA_TDNN, self).__init__()
        self.scale  = model_scale
        self.conv1  = nn.Conv1d(num_mels, C, kernel_size=5, stride=1, padding=2)
        self.relu   = nn.ReLU()
...
def MainModel(eca_c=1024, eca_s=8, log_input=True, num_mels=80, num_out=192, **kwargs):
    model = ECAPA_TDNN(block=Bottle2neck, C=eca_c, model_scale=eca_s, log_input=log_input, num_mels=num_mels, num_out=num_out, **kwargs)
    return model
